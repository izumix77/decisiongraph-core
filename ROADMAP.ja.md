...
- **ノードステータス遷移操作** — `Active → Superseded` の明示的な遷移
  - 現在、コミット済みノードのステータスを変更する手段がない
  - 「時間経過で無効になる決定」を表現するために必要
  - 主なユースケース: 時系列ADRのsupersessionシナリオ
  - 実現により `supersede_node` がファーストクラスのカーネル操作になる
...
- 可視化 / 解析ツール
  - `graph <directory>` — 依存グラフ全体の可視化（違反だけでなく正常な依存チェーンも表示）
  - 正常な依存関係も含めた構造の閲覧
