import { buildAjv, schemaIdV02, schemaIdV03, schemaIdV04 } from "./ajv.js";
...
export type DecisionLogJsonV04 = { version: "0.4"; graphId: string; ops: unknown[] };
export type DecisionLogJson = DecisionLogJsonV02 | DecisionLogJsonV03 | DecisionLogJsonV04;
...
export function validateDecisionJson(input: unknown):
  ...
  if (version === "0.4") {
    const validate = ajv.getSchema(schemaIdV04);
    if (!validate) return { ok: false, errors: ["Validator v0.4 not found"] };
    const ok = validate(input);
    if (!ok) return { ok: false, errors: formatAjvErrors(validate.errors) };
    return { ok: true, value: input as DecisionLogJsonV04 };
  }
