...
> 仕様・解釈・運用上の正式な根拠は、英語版 README.md および Constitution v0.4 を正とします。
...
### Supersession モデル（v0.4）

ノードの Supersession は **トポロジーから導出**されます（保存されません）：

- `Node` は `status` フィールドを持ちません
- `effectiveStatus(store, nodeId)` は `supersedes` エッジを走査し、`"Active"` または `"Superseded"` を返します
- これにより、「命題が自身の否定を含む」というカテゴリエラーを排除しています

エッジの Supersession は引き続き公理的（axiom）です：

- `Edge.status` は `"Active"` または `"Superseded"` のいずれか（`"Deprecated"` は廃止）
- `supersede_edge` は **アトミック** — 旧エッジの Superseded 化と新エッジの追加が単一操作
...
Decision A
 └─ depends_on → Decision B
      └─ supersedes → Decision C   (C はトポロジー由来の Superseded)
```
...
規範上の正本（source of truth）は `docs/constitution/v0.4` です。
ドキュメントと実装の間に矛盾がある場合、
...
- **README.md** — 概要（英語正本）
- **Constitution v0.4** — 規範要件（最上位）
- **JSON Schema v0.4** — データ検証仕様
- **Minimal Kernel API v0.4** — 実装仕様
...
v0.4 では、`Node.status` を廃止し（Supersession はトポロジー導出の `effectiveStatus` に一本化）、
`EdgeStatus` を二値（`Active` | `Superseded`）に简略化、`overrides` エッジタイプを廃止、
`supersede_node` オペレーションを廃止しました。
...
- **現在のバージョン:** v0.4 (core@0.4.2, schema@0.2.0, io-json@0.2.0, cli@0.1.3)
- **安定性:** アクティブ開発中
- **規範上の最終権威:** Constitution v0.4
